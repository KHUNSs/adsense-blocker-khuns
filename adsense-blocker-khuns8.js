...
(function() {
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    // 블로그 주인이 설정할 수 있는 변수들
    // 이 부분을 수정하여 원하는 링크 설정 가능
    const customRedirectUrl = ''; // 비워두면 현재 페이지에 머무름
    ...
    function createCustomPopup(message, type) {
      ...
      popupElement.innerHTML = `
        <div class="custom-popup-container">
          ...
          <div class="custom-popup-footer">Created by KHHNS</div>
        </div>
      `;
      ...
      // 확인 버튼 (계속 이용하기/지금 확인하기) - 현재 페이지 유지 또는 사용자 지정 URL로 이동
      button1.onclick = function() {
        popupElement.remove();
        localStorage.removeItem('popupVisible');
        
        // 사용자 지정 URL이 있으면 이동, 없으면 현재 페이지 유지
        if (customRedirectUrl && customRedirectUrl.trim() !== '') {
          window.location.href = customRedirectUrl;
        }
      };
      
      // 취소 버튼 (다른 곳으로 이동) - 항상 지정된 URL로 이동
