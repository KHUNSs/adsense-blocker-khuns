// adsense-blocker-khuns9.js
// 광고 클릭, 새로고침 횟수를 추적하는 코드 - 전면 팝업 버전
(function() {
  // 페이지가 로드될 때 실행
  document.addEventListener('DOMContentLoaded', function() {
    ...
    function createCustomPopup(message, type) {
      ...
      const buttonText1 = '확인';
      const buttonText2 = '취소';
      ...
      popupElement.innerHTML = `
        <div class="custom-popup-container">
          ...
          <div class="custom-popup-footer">
            <img src="https://blog.kakaocdn.net/dn/A0tKP/btsN4c6NVCl/771X7bQ6YvGF8TJa1Bqru0/img.png" alt="KHUNS Logo" class="custom-popup-logo">
            Created by KHUNS
          </div>
        </div>
      `;
      ...
      // 확인 버튼 - 사용자 지정 URL로 이동하거나 현재 페이지 유지
      button1.onclick = function() {
        popupElement.remove();
        localStorage.removeItem('popupVisible');
        
        // 사용자 지정 URL이 있으면 이동 (redirectTarget)
        if (window.redirectTarget && window.redirectTarget.trim() !== '') {
          window.location.href = window.redirectTarget;
        }
      };
      
      // 취소 버튼 - 항상 고정된 URL로 이동
      button2.onclick = function() {
        window.location.href = 'https://wp3.nomoneynosunny.com';
      };
